...
def filter_df(dff,exchange, margin, start_date, end_date):
    ...
    mask3 = ((dff['Entry time'] > start_date) & (dff['Entry time'] <= end_date))
    # print(dff[(mask1 & mask2 & mask3)])
    return dff[(mask1 & mask2 & mask3)]

def calc_returns_over_month(dff):
    out = []
    dff['YearMonth']= pd.to_datetime(dff['Entry time']).dt.to_period('M')
...
def update_dates_due_to_exchange_select(value):
    ...
    #print(df_aux['Entry time'].min(),df_aux['Entry time'].max())
    return df_aux['Entry time'].min(),df_aux['Entry time'].max()
